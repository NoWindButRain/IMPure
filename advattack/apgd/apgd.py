...
class APGD(Attack):
    ...
    def __init__(self, model, dataloader, save_path, criterion,
            eps=8, steps=10, ieps=2,
            norm='Linf', loss='ce', n_restarts=1, eot_iter=1, rho=.75):
        _name = 'APGD_{}_{}_{}_{}'.format(eps, steps, ieps, loss)
        super(APGD, self).__init__(_name, model, dataloader, save_path)
    ...
    def attack(self, images, labels=None, best_loss=False):
        # random_start
        x = images.clone()
        y = labels.clone()
        adv = x.clone()
        acc = self.model(images).argmax(1) == labels
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        self.steps_2, self.steps_min, self.size_decr = \
            max(int(0.22 * self.steps), 1), max(int(0.06 * self.steps), 1), \
