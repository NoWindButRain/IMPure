...
import recon_model as models_mae
...
def get_args_parser():
    ...
    parser.add_argument('--train_batch_size', default=[64], nargs='+', type=int,
    ...
    # Dataset parameters
    parser.add_argument('--wds_data_urls', default=[], nargs='+', type=str,
                        help='webdataset data urls')
    parser.add_argument('--wds_data_total', default=50000, type=int,
                        help='webdataset data total')
...
def main(args):
    ...
    transform_train = transforms.Compose([
        # transforms.RandomChoice(_trans_rand_rc),
    ...
    _trans_rand_rc = [
        [transforms.RandomResizedCrop(
            i, scale=(0.2, 1.0), interpolation=3, antialias=None), j]
        for i, j in zip(args.crop_size, args.train_batch_size)]

    def rc_batchsampler(src):
        _rc, batch_size = _trans_rand_rc[np.random.randint(len(_trans_rand_rc))]
        batch = [0] * batch_size
        idx_in_batch = 0
        for sample in src:
            image, label = sample
            batch[idx_in_batch] = [_rc(image), torch.Tensor([label])]
            idx_in_batch += 1
    ...
    # webdatset or ImageFolder dataset
    end_epoch = int(
        args.wds_data_total / misc.get_world_size() / args.num_workers /
        np.mean(args.train_batch_size)
    )
    length = end_epoch * args.num_workers
    from dataset import ImageNetWDS
    dataset_train = ImageNetWDS(
        args.wds_data_urls, transform=transform_train,
        seed=args.seed,
        length=length, end_epoch=end_epoch, epoch=args.start_epoch,
        rc_batchsampler=rc_batchsampler
    )
    print(dataset_train.pipeline)
    print('dataset train num: ', len(args.wds_data_urls))
    ...
    eff_batch_size = np.mean(args.train_batch_size) * \
    ...
    for epoch in range(args.start_epoch, args.epochs * args.mul_epochs):
        if args.distributed:
            _seed = epoch + args.seed
            data_loader_train.dataset.pipeline[0].seed = _seed
            data_loader_train.dataset.pipeline[3].epoch = epoch
